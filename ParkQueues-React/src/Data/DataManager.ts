...
export let AttractionMap = new Map<string, Attraction>();
...
export class DataManager {
  static async fetchDestination(slug: string): Promise<Destination | null> {
    ...
    if (localData && Date.now() - (1000 * 60 * 5) < JSON.parse(localData).lastUpdated) {
      ...
      try {
        ...
        if (newDest) {
          ...
          if (!existingDest) {
            ...
            for (const [parkKey, value] of Object.entries(existingDest)) {
