...
export default function SingleLineAreaChart({ data, timezone }:
  { data: { date: number; high: number | undefined }[], timezone: string }) {
  const DATA = data;
  ...
  const activeDate = useDerivedValue(() => {
    ...
    // One-touch only
    if (!isSecondPressActive) return new Date(firstTouch.x.value.value).toLocaleString(
      'en-US', {
        hour: 'numeric',
        minute: 'numeric',
        hour12: true,
  ...
  const activeHigh = useDerivedValue(() => {
    ...
    if (!isSecondPressActive) {
      let val = firstTouch.y.high.value.value.toFixed(0);
      switch (val) {
        case 'NaN':
          return "Closed";
        case '1':
